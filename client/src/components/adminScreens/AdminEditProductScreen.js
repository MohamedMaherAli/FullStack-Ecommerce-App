...
function AdminCreateProductScreen() {
  ...
  const { product, loading: loadingProductDetails } = productDetailsState;

  const productUpdateState = useSelector((state) => state.productUpdateReducer);
  const { success: successUpdate, error: errorUpdate } = productUpdateState;
  ...
  useEffect(() => {
    if (product._id !== id) {
      ...
      formik.setFieldValue('mainCategory', product.mainCategory);
      formik.setFieldValue('category', product.category);
      formik.setFieldValue('countInStock', product.countInStock);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm, setSubmitting }) => {
      setSubmitting(false);
      dispatch(updateProduct(id, { ...values, onSale }));
      resetForm();
    },
  });
